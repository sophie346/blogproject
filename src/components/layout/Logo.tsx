...
export async function Logo({ href, className }: LogoProps) {
  ...
  const configuredHome = String(brand.homeHref || "").trim();
  const homeHref = href || configuredHome || (await siteHref("/"));
  const isExternal = /^https?:\/\//i.test(homeHref);
  const classNames =
    `site-logo inline-flex shrink-0 items-center gap-2.5 ${className || ""}`.trim();
  ...
  if (isExternal) {
    return (
      <a href={homeHref} className={classNames}>
        {mark}
      </a>
    );
  }
  ...
  return (
    <Link href={homeHref} className={classNames}>
      {mark}
    </Link>
  );
}
